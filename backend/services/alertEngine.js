  ...
  try {
    ...
    for (const rule of rules) {
      try {
        ...
        const metricsDoc = await Metric.find({ _id: { $in: metricIds } });
        const metricsMap = Object.fromEntries(
            metricsDoc.map(m => [m._id.toString(), m.value])
        );
        ...
        if (breach && shouldTrigger) {
          // Use rule.user directly (it's an ObjectId)
          ...
          await triggerNotification(rule, null);
