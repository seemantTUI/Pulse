...
const createMetric = async (req, res) => {
    ...
    if (missingFields.length > 0) {
        return res.status(400).json({ error: 'Please fill in all the fields', emptyFields: missingFields });
    }

    try {
        const metric = await Metric.create({ metricName, value });
        // Only emit on create
        appEvents.emit('dataChanged');
...
const updateMetric = async (req, res) => {
    ...
    try {
        const metric = await Metric.findOneAndUpdate({ _id: id }, { ...req.body }, { new: true });
        if (!metric) {
            return res.status(404).json({ error: `Metric with ${id} does not exist` });
        }
        // Only emit on update
